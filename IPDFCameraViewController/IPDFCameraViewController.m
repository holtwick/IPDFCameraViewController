...
@implementation IPDFRectangleFeature @end
...
- (void)awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(_backgroundMode)
                                                 name:UIApplicationWillResignActiveNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(_foregroundMode)
...
{
    if (self.forceStop || _isStopped || _isCapturing || !CMSampleBufferIsValid(sampleBuffer)) {
        return;
    }
    ...
    if (self.isBorderDetectionEnabled) {
        if (_borderDetectFrame) {
            _borderDetectLastRectangleFeature = [self biggestRectangleInRectangles:[[self rectangleDetector] featuresInImage:image]];
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        ...
        @autoreleasepool {
            ...
            if (weakSelf.isBorderDetectionEnabled && rectangleDetectionConfidenceHighEnough(_imageDedectionConfidence)) {
                CIRectangleFeature *rectangleFeature = [self biggestRectangleInRectangles:[[self rectangleDetector] featuresInImage:enhancedImage]];
                if (rectangleFeature) {
...
- (CIRectangleFeature *)biggestRectangleInRectangles:(NSArray *)rectangles {
    ...
    NSArray *points = @[ [NSValue valueWithCGPoint:rectangleFeature.topLeft],
                         [NSValue valueWithCGPoint:rectangleFeature.topRight],
                         [NSValue valueWithCGPoint:rectangleFeature.bottomLeft],
                         [NSValue valueWithCGPoint:rectangleFeature.bottomRight]];
